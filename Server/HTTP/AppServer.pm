...
use warnings;
use Data::Dumper;
use Server::HTTP::AppServer::Base;
...
sub init
{
	...
	$self->{'server'} = Server::HTTP::AppServer::Base->new($self->{'ServerPort'});
...
sub plugin
{
	...
	# load module
	eval('use Server::HTTP::AppServer::Plugin::'.$name);
	die "Failed to load plugin '$name': $@\n" if $@;
	
	# call init() method of module to get the handlers of the plugin
	eval('$self->handle( Server::HTTP::AppServer::Plugin::'.$name.'->init($self->{"server"}, %options))');
