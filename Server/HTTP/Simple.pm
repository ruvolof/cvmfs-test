...
Server::HTTP::Simple - Lightweight HTTP server
...
=head1 SYNOPSIS
 ...
 use Server::HTTP::Simple;
 
 my $server = Server::HTTP::Simple->new();
 $server->run();

However, normally you will sub-class the Server::HTTP::Simple::CGI
module (see L<Server::HTTP::Simple::CGI>);

 package Your::Web::Server;
 use base qw(Server::HTTP::Simple::CGI);
...
in order to provide restart-on-SIGHUP, Server::HTTP::Simple sets a SIGHUP
...
=head2 Server::HTTP::Simple->new($port)
...
sub new {
    ...
    if ( $class eq __PACKAGE__ ) {
        require Server::HTTP::Simple::CGI;
        return Server::HTTP::Simple::CGI->new( @_[ 1 .. $#_ ] );
    }
    ...
    *{"$pkg\::process_request"} = $self->_process_request;
    ...
    if ($server) {
        ...
        *{"$pkg\::ISA"} = [$server];
        ...
        require Server::HTTP::Simple::CGI;
        *{"$pkg\::post_accept"} = sub {
            Server::HTTP::Simple::CGI::Environment->setup_environment;
        ...
        while ($SERVER_SHOULD_RUN) {
            ...
            while ( accept( my $remote = new FileHandle, HTTPDaemon ) ) {
                ...
                *STDOUT = $self->stdout_handle();
                select STDOUT;   # required for Server::HTTP::Simple::Recorder
