package Server::HTTP::Simple::CGI;

use base qw(Server::HTTP::Simple Server::HTTP::Simple::CGI::Environment);
use strict;
use warnings;

use vars qw($VERSION $default_doc $DEFAULT_CGI_INIT $DEFAULT_CGI_CLASS);
$VERSION = $Server::HTTP::Simple::VERSION;
...
Server::HTTP::Simple::CGI - CGI.pm-style version of Server::HTTP::Simple

=head1 DESCRIPTION

Server::HTTP::Simple was already simple, but some smart-ass pointed
...
See the docs in L<Server::HTTP::Simple> for more detail.
...
Handler implemented as part of Server::HTTP::Simple API
...
<html>
  ...
  <body>
    ...
    <p>You now have a functional Server::HTTP::Simple::CGI running.
      </p>

    <p><i>(If you're seeing this page, it means you haven't subclassed
      Server::HTTP::Simple::CGI, which you'll need to do to make it
      useful.)</i>
      </p>
  </body>
</html>
